...
let farmingListLength, mintingListLength
let anyRate, chngRate
let categories, odds
...
const assetConfig = () => {
  ...
  farmListLength = farmingAssets.length
  mintListLength = mintingAssets.length
  anyRate = "0.00001"
  chngRate = "0.00001"
...
contract("AirdropV2 Contract", async () => {
  ...
  it("get correct rewards for a position", async () => {
    ...
    await subscribe(user)
    const { asset, amount } = await stakeThis()
    const now = new BigNumber((await web3.eth.getBlock("latest")).timestamp)
    await airdropV2.stake(asset, amount, { from: user })
    await advanceBlockAtTime(now.plus(oneYear).toNumber())
    const actualRewards = Number(utils.fromWei(await airdropV2.getFarmRewards(user, asset)))
    const expectedRewards = oneYear.multipliedBy(anyRate).multipliedBy(utils.fromWei(amount))
    expect(actualRewards.toFixed(0)).to.equal(expectedRewards.toFixed(0))
  })

  // get mint rewards
  it("get correct rewards for a potential position", async () => {
    await setAll()
    await setTerm(fromNowOneYear)
    const { asset, amount } = await lockThis()
    const now = new BigNumber((await web3.eth.getBlock("latest")).timestamp)
    const termEnd = await airdropV2.termEnd(LONG)
    const expectedRewards = oneYear.multipliedBy(chngRate).multipliedBy(utils.fromWei(amount))
    const actualRewards = Number(utils.fromWei(await airdropV2.getMintRewards(asset, amount, termEnd)))
    expect(actualRewards.toFixed(0)).to.equal(expectedRewards.toFixed(0))
  })

  // free to fmn
  it("gets correct conversion from free to fmn", async () => {
    await setAll()
    const reserves = await pool.getReserves()
    const { _reserve0, _reserve1 } = reserves
    const freeReserve = new BigNumber(_reserve0)
    const fmnReserve = new BigNumber(_reserve1)
    const rate = fmnReserve.dividedBy(freeReserve)
    const oneHundredFree = rate.multipliedBy("100")
    const actualConversion = Number(utils.fromWei(await airdropV2.freeToFmn(utils.toWei("100"))))
    expect(oneHundredFree.toFixed(15)).to.equal(actualConversion.toFixed(15))
  })

  // get position id
  it("gets a valid hash of a position", async () => {
    await setAll()
    await setTerm(fromNowOneYear)
    const { asset } = await lockThis()
    const termEnd = await airdropV2.termEnd(LONG)
    const hash = await airdropV2.getPositionId(user, asset, termEnd)
    const isHash = web3.utils.isHexStrict(hash) && hash.length === 66
    expect(isHash).to.be.true
  })

  // farming asset count
  it("gets correct farm list length", async () => {
    await setAll()
    const farmLength = (await airdropV2.farmingAssetCount()).toNumber()
    expect(farmLength).to.equal(farmListLength)
  })

  // minting asset count
  it("gets correct mint list length", async () => {
    await setAll()
    const mintLength = (await airdropV2.mintingAssetCount()).toNumber()
    expect(mintLength).to.equal(mintListLength)
  })

  // term shift
  it("term shift should not affect positions", async () => {
    await setAll()
    await transferMintAsset(user, "100")
    await approveAirdrop(user)
    await subscribe(user)
    await setTerm(fromNowFourMonths)
    const { asset, amount } = await lockThis()
    const initialTermEnd = await airdropV2.termEnd(LONG)
    const initialPosId = await airdropV2.getPositionId(user, asset, initialTermEnd)
    await airdropV2.lock(asset, amount, LONG, { from: user })
    const initialBal = utils.fromWei(await airdropV2.positionBalance(initialPosId))
    await airdropV2.newTerm(fromNowEightMonths, { from: governance })
    const firstShiftTermEnd = await airdropV2.termEnd(MEDIUM)
    const firstShiftPosId = await airdropV2.getPositionId(user, asset, firstShiftTermEnd)
    const firstShiftBal = utils.fromWei(await airdropV2.positionBalance(firstShiftPosId))
    await airdropV2.newTerm(fromNowOneYear, { from: governance })
    const secondShiftTermEnd = await airdropV2.termEnd(SHORT)
    const secondShiftPosId = await airdropV2.getPositionId(user, asset, secondShiftTermEnd)
    const secondShiftBal = utils.fromWei(await airdropV2.positionBalance(secondShiftPosId))
    expect(firstShiftPosId).to.equal(initialPosId)
    expect(secondShiftPosId).to.equal(initialPosId)
    expect(firstShiftBal).to.equal(initialBal)
    expect(secondShiftBal).to.equal(initialBal)
  })
})