...
contract FMN is Controller, FRC759 {
    ...
    constructor (string memory _name, string memory _symbol, uint8 _decimals, uint256 _maxSupply, address _faucet)
    ...
    function burn(address account, uint256 amount) public {
        require(msg.sender == account, "FREEMOON: Only your own tokens.");
        _burn(account, amount);
    }
    ...
    function rewardWinner(address _winner, uint8 _lottery) external {
        require(msg.sender == faucet, "FREEMOON: FMN can only be won from the faucet.");
        _mint(_winner, reward);
