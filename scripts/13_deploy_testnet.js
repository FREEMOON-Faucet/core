...
const COORDINATOR = process.env.TESTNET_COORD
const GOV = process.env.TESTNET_GOV
...
const deploy = async () => {
  ...
  try {
    console.log(`###### Pool ######`)
    const nextNonce = await web3.eth.getTransactionCount(admin)
    pool = await ChaingeDexPair.new({ from: admin, gas: 8000000, gasPrice: 11000000000, nonce: nextNonce + 2 })
